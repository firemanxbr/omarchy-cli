...
# xz is served by 'packages' for x86_64 → refused there; aarch64 has nobody → queued.
...
tid=$(python3 -c 'import json,sys; print(json.load(sys.stdin)["task"]["id"])' <<<"$claim")
...
[[ "$(curl -s -o /dev/null -w '%{http_code}' -X POST "$OMARCHY_API/api/v1/factory/tasks/$tid/complete" "${fauth[@]}" -d '{"worker":"w2","sha256":"0000","filename":"x"}')" == 409 ]] || { echo "complete before publish must be refused"; exit 1; }
# The "build result": the xz fixture published as source factory for aarch64.
"$PKG_REPO" publish --ring edge --source factory --arch aarch64 --note "factory e2e" "$E2E/pkgs/xz-5.8.4-1-x86_64.pkg.tar.zst" >/dev/null
