...
__CHARTS__
  ...
  function renderOps(d) {
    ...
    var card = function (cls, name, ws, blurb) {
      ...
      return '<div class="role ' + cls + '"><h3>' + name + '<span>' + num(ws.length) + (ws.length === 1 ? " worker" : " workers") + '</span></h3><p>' + blurb + '</p><div class="u"><span class="dim">building now</span><div class="bar" style="height:10px;background:var(--panel-2);border:1px solid var(--line);position:relative;display:block"><i style="position:absolute;left:0;top:0;bottom:0;width:' + (al.length ? 100 * bz.length / al.length : 0) + '%;background:' + col + '"></i></div><b class="num">' + num(bz.length) + ' / ' + num(al.length) + '</b></div>' +
