...
const BODY = String.raw`
  ...
  <section>
    ...
    <div class="steps">
      ...
      <div class="step"><h3>Maintainer of a group</h3><p>A contributor listed under that group. Approves or rejects the staged builds of the group with the evidence in front of them, then <b>writes the project's recipe</b> from that evidence — the log, the metrics, the contributor's PKGBUILD are what they learn from, never what they ship — and opens the pull request that adds it to <code>factory/pkgbuilds/&lt;group&gt;/&lt;name&gt;/</code>; the merge is what the project builds, signs and publishes. <b>Never their own package</b>: another maintainer of the group approves what a maintainer brought, and a group with a single maintainer is no exception — that maintainer's own packages wait for a second one. Trusts workers as project workers; reviews governance pull requests.</p></div>
