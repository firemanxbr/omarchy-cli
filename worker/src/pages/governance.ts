...
const BODY = String.raw`
  ...
  <section>
    ...
    <div class="steps">
      ...
      <div class="step"><h3>Agent keys stay with the worker's owner</h3><p>If a worker drafts or corrects PKGBUILDs with an agent (community trust), or audits staged builds for the maintainers (project trust), the key is its owner's: <code>ANTHROPIC_API_KEY</code> in the container's environment when it starts. The pool holds no agent key and GitHub runs no agent — the hosted fallback never takes an audit; what an agent produces is evidence like any other build, reviewed by a maintainer before it reaches anyone.</p></div>
