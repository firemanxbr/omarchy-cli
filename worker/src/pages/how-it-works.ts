...
const BODY = String.raw`
  ...
  <section id="factory">
    ...
    <p class="sub">Three personas, three costs. <b>Users</b> only see the pool: signed databases, health checks, 48 hours from <code>edge</code> to <code>stable</code>, rollback. <b>Contributors</b> have something to package: they register it (a GitHub login and the project's URL — nobody to ask, nothing spent by the project), run the signed <code>omarchy-worker</code> container wherever they like, with their own agent keys, and the build lands in <em>their</em> staging workspace with the PKGBUILD and the log. <b>Maintainers</b>, by group, approve staged builds with the evidence in front of them — and never use the contributor's bytes: an approved package is rebuilt from the same recipe on a worker the project trusts, signed by the pool, and enters <code>edge</code> as source <code>factory</code> to take the same 48-hour path as everything else. <em>We do not use what you built, we learn from it</em>: the recipe, the log and the metrics a contributor produced with the same tools (and, if they like, their own agent) make the maintainer's own build faster and its approval surer, so every package users get was checked by two different people. New upstream releases of an approved package are built again on the contributor's worker and reviewed again. Who the maintainers are, and how one becomes one, is a file in the repository changed by pull request — see <a href="/docs/governance">Governance</a>. Every stage is on the <a href="/factory">Factory</a> page.</p>
  </section>
