...
const BODY = String.raw`
  ...
  <section>
    ...
    <div class="table-wrap"><table><thead><tr><th>Endpoint</th><th>What it returns</th></tr></thead><tbody>
      ...
      <tr><td><code>GET /factory/approvals</code> · <code>/groups</code> · <code>/trust</code> · <code>/blocks</code></td><td>The record: every decision with who signed it; the groups and their maintainers (from <code>factory/MAINTAINERS.toml</code>); project-trusted workers; what is blocked now and why.</td></tr>
...
curl -s  https://pool.firemanxbr.org/core/x86_64/omarchy-core-stable.db | tar -tz | head</pre></div>
  ...
  <section>
    ...
    <div class="table-wrap"><table><thead><tr><th>Endpoint</th><th>Who</th><th>What it does</th></tr></thead><tbody>
      ...
      <tr><td><code>POST /factory/tasks/:id/build</code> · <code>/approve</code> · <code>/reject</code></td><td>maintainer of the group</td><td>Have the project build a contributor's staged package again (its agent, a trusted worker, its own recipe); approve the project's build into edge — the decision on the record, a publish job; or send either back with a note. Never your own package.</td></tr>
