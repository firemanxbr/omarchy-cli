  ...
  <div id="signed" hidden>
    <section>
      ...
      <form id="pkg-form" class="form" onsubmit="return false">
        ...
        <label>Group <select id="pkg-group"><option value="community">community</option><option value="omarchy">omarchy</option></select></label>
    ...
    <section>
      ...
      <p class="sub">Builds happen on your machine, with your resources (and your agent's key, if you want PKGBUILDs drafted and corrected for you). Register a worker, then run the signed image with the token it gives you — shown once. <b>Dedicated</b> builds only your packages; <b>shared</b> also helps others.</p>
      <form id="worker-form" class="form" onsubmit="return false">
        ...
        <label>Mode <select id="w-mode"><option value="dedicated">dedicated (my packages)</option><option value="shared">shared (anyone's)</option></select></label>
...
const SCRIPT = String.raw`
  ...
  $("#worker-form").onsubmit = function () {
    var body = { name: $("#w-name").value.trim(), arch: $("#w-arch").value, mode: $("#w-mode").value };
    $("#w-btn").disabled = true;
