...
| `GET /api/v1/packages/:sha256/provenance` | the seal of an object: a synced package's upstream project and the keyring its signature was verified against; a factory package's chain — the evidence build, the audit, the approval, the rebuild — and the attestation the pool wrote next to the object (`<filename>.provenance.json`, an in-toto Statement, `.sig` by the pool's key; `routes/seal.ts`, written when the project's build completes, removed with the object by GC) |
